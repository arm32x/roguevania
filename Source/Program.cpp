...
void Program::main(int argc, char** argv) {
    ...
    while (window.isOpen()) {
        ...
        for (; substeps > 0; substeps--) {
            player.update(delta);
            camera.update(delta, player.getPosition() + Vector2f(player.getTextureRect().width / 2 + (Keyboard::isKeyPressed(Keyboard::D) ? 80 : Keyboard::isKeyPressed(Keyboard::A) ? -80 : 0), player.getTextureRect().height / 2));
        }
