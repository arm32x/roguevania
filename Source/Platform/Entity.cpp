    ...
    for (TilemapCollider* collider : TilemapCollider::all) {
        ...
        switch (mode) {
            ...
            case CollisionMode::Solid: {
                ...
                onGround = true;
                for (float amountMoved = 0.0f; (mode == CollisionMode::Solid || mode == CollisionMode::NotTouching) && std::abs(amountMoved) <= std::abs(velocity.y); amountMoved += velocity.y >= 0.0f ? -increment : increment) {
                ...
                if (mode == CollisionMode::Solid || mode == CollisionMode::NotTouching) {
                    onGround = false;
                    mode = collider->prioritizeTileModes(collider->getTilesTouching(*this));
                    for (float amountMoved = 0.0f; (mode == CollisionMode::Solid || mode == CollisionMode::NotTouching) && std::abs(amountMoved) <= std::abs(velocity.x); amountMoved += velocity.x >= 0.0f ? -increment : increment) {
