        ...
        switch (mode) {
            ...
            case CollisionMode::Solid: {
                ...
                if (mode == CollisionMode::Solid) {
                    ...
                    mode = collider->prioritizeTileModes(collider->getTilesTouching(*this));
                    if (mode == CollisionMode::Solid || (mode == CollisionMode::SemiSolid && !Keyboard::isKeyPressed(Keyboard::S))) {
                        onGround = true;
                        for (float amountMoved = 0.0f; (mode == CollisionMode::Solid || (mode == CollisionMode::SemiSolid && !Keyboard::isKeyPressed(Keyboard::S))) && std::abs(amountMoved) <= std::abs(velocity.y * delta); amountMoved += velocity.y >= 0.0f ? -increment : increment) {
                            move(0.0f, velocity.y >= 0.0f ? -increment : increment);
                            setVelocity(velocity.x, 0.0f);
                            mode = collider->prioritizeTileModes(collider->getTilesTouching(*this));
                        }
