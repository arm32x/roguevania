    ...
    for (TilemapCollider* collider : TilemapCollider::all) {
        ...
        switch (mode) {
            ...
            case CollisionMode::Solid: {
                ...
                for (amountMoved.y = 0.0f; (mode == CollisionMode::Solid || mode == CollisionMode::SolidFloor) && std::abs(amountMoved.y) <= std::abs(velocity.y) - (velocity.y >= 0.0f ? -0.125f : 0.125f) * 2; amountMoved.y += velocity.y >= 0.0f ? -0.125f : 0.125f) {
                    move(0.0f, velocity.y >= 0.0f ? -0.125f : 0.125f);
